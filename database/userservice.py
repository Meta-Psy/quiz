...
# функция вывода результата пользователя
def result_db(user_id):
    with next(get_db()) as db:
        exact_user = db.query(Rating).filter_by(user_id=user_id).one()
        if exact_user:
            result = exact_user.correct_answers
            return result
        else:
            return None


# функция определения места в общем рейтинге
def rating_db(level, user_id):
    with next(get_db()) as db:
        users_rating = db.query(Rating).filter_by(level=level).order_by(Rating.correct_answers.desc).all()
        count = 0
